...
/**
 * @namespace com.sovanta.MyComplexUI5TSTest
 */
export default class Component extends UIComponent {
  ...
  public init(): void {
    ...
    this.getRouter().attachBeforeRouteMatched(
      (event: UI5Event) => this.onBeforeRouteMatched(event),
      this
    );
  ...
  private getFcl(): Promise<FlexibleColumnLayout> {
    return new Promise((resolve) => {
      const FCL = (this.getRootControl() as View).byId(
        'fcl'
      ) as FlexibleColumnLayout;
      if (!FCL) {
        (this.getRootControl() as View).attachAfterInit((oEvent: UI5Event) => {
          resolve(
            (oEvent.getSource() as View).byId('fcl') as FlexibleColumnLayout
          );
        });
        return;
      }
