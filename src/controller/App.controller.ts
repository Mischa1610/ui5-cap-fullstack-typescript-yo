...
export default class App extends BaseController {
  ...
  public onInit(): void {
    ...
    ownerComponent
      .getRouter()
      .attachRouteMatched(
        (event: UI5Event) => this.onRouteMatched(event),
        this
      );
  }

  public onStateChanged(oEvent: UI5Event): void {
    const bIsNavigationArrow = oEvent.getParameter(
        'isNavigationArrow'
      ) as string,
    ...
    if (bIsNavigationArrow) {
      (this.getOwnerComponent() as AppComponent)
        ...
        .navTo(
          ...
          { layout: sLayout, id: this.currentId },
          {},
          true
        );
    }
  ...
  private async updateUIElements() {
    const oModel = (this.getOwnerComponent() as AppComponent).getModel(
        'appView'
      ) as JSONModel,
